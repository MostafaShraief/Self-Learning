...
SELECT ID,Engine
FROM VehicleDetails
WHERE EXISTS (SELECT Top 1 x = 'a' FROM FuelTypes WHERE  FuelTypes.FuelTypeID = VehicleDetails.FuelTypeID AND fueltypeid  = 1)
order by Id;

Select v.ID, v.Engine From
VehicleDetails v join FuelTypes f
on v.FuelTypeID = f.FuelTypeID
where f.FuelTypeID = 1
order by v.ID
